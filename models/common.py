...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False).to(self.device)
        self.bn = nn.BatchNorm2d(c2).to(self.device)
...
def initialize_orthogonal_filters(c, h, w):
    if h * w < c:
        n = c // (h * w)
        gram = [gram_schmidt(torch.rand([h * w, 1, h, w], device='cuda')) for _ in range(n)]
        return torch.cat(gram, dim=0)
    else:
        return gram_schmidt(torch.rand([c, 1, h, w], device='cuda'))
...
class channel_shuffle(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        b, c, h, w = x.size()
        g = self.num_groups
        assert c % g == 0, "Number of channels must be divisible by groups"
...
class SpatialAttention(nn.Module):
    def __init__(self, in_channels: int):
        ...
        self.conv = nn.Conv2d(in_channels * 2, 1, kernel_size=1).cuda()
    ...
    def forward(self, x):
        ...
        x = self.conv(x)
        return original * self.sigmoid(x)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, height, stride=1, downsample=None, num_groups=4):
        ...
        self._excitation = nn.Sequential(
            nn.Linear(planes // num_groups, round(planes / 16), bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(round(planes / 16), planes // num_groups, bias=False),
            nn.Sigmoid(),
        ).cuda()
        self.OrthoAttention = Attention()
        self.F_C_A = GramSchmidtTransform.build(planes // num_groups, height)
        self.channel_shuffle = channel_shuffle(num_groups)
        self.spatial_attention = SpatialAttention(planes)

    def forward(self, x):
        if x.size(1) % self.num_groups != 0:
            self.num_groups = x.size(1)
        groups = torch.split(x, x.size(1) // self.num_groups, dim=1)
        out = []
        for group in groups:
            compressed = self.OrthoAttention(self.F_C_A, group)
            b, c = group.size(0), group.size(1)
            excitation = self._excitation(compressed).view(b, c, 1, 1)
            out.append(excitation * group)
        concatenated = torch.cat(out, dim=1)
        shuffled = self.channel_shuffle(concatenated)
        return self.spatial_attention(shuffled)
...
class C3(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.cv1(x).cuda()
        x2 = self.cv2(x).cuda()
        x3 = self.m(self.cv1(x)).cuda()
        out = torch.cat((self.basicblock(x3), x2), 1).cuda()
        return self.cv3(out).cuda()
