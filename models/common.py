...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False).to(self.device)
        self.bn = nn.BatchNorm2d(c2).to(self.device)
        self.act = self.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity().to(self.device)
    def forward(self, x):
